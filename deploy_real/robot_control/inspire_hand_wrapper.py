...
class InspireHandController:
    def __init__(self, left_ip='192.168.123.210', right_ip='192.168.123.211',
        ...
        if re_init:
            self.left_client.write_register(REG_CLEAR_ERROR, 1, unit=self.device_id)
            self.right_client.write_register(REG_CLEAR_ERROR, 1, unit=self.device_id)
    ...
    def _read_registers_signed(self, client, address, count):
        ...
        try:
            response = client.read_holding_registers(address, count, unit=self.device_id)
    ...
    def ctrl_dual_hand(self, left_q_target, right_q_target):
        ...
        try:
            self.left_client.write_registers(REG_ANGLE_SET, left_angles, unit=self.device_id)
            self.right_client.write_registers(REG_ANGLE_SET, right_angles, unit=self.device_id)
        except Exception as e:
